...
def cli() -> argparse.Namespace:
    ...
    # add subparsers; for now only antismash
    subparsers = parser.add_subparsers(dest="command", required=True)
    antismash_parser = subparsers.add_parser("antismash", help="parse antiSMASH GenBank files")
    antismash_parser.add_argument("--gbks", type=str, required=True, help="directory containing antiSMASH GenBank files")
    antismash_parser.add_argument("--out", type=str, required=True, help="output directory")
    antismash_parser.add_argument("--paras", type=str, required=False, help="path to all-substrates PARAS model file (optional)")
    antismash_parser.add_argument("--cache", type=str, required=False, help="cache directory (optional)")
    antismash_parser.add_argument("--hmms", type=str, required=False, help="directory with HMMs for gene models (optional)")
    antismash_parser.add_argument("--workers", type=int, default=1, help="number of parallel workers to use")
...
def main() -> None:
    ...
    add_file_handler(os.path.join(args.out, "biocracker.log"), level="INFO")
